...
LT_w_IT2MF = [0 0 0 0.3333 0 0 0 0.1667 1; % linguistic weights of linguistic terms
...
criteria_weights = [ 0 0 0.0833 0.4166 0 0 0.0833 0.25 0.9; % linguistic weights of criteria
...
%% IT2 Envelope calculation

D_by_C_by_A_envelope = main;
...
% %                 fuzziness = fuzziness + fuzziness(Y);
%                 Y = [];
%             end
%             if(i == 1)
%                 subplot(m,n,count);
...
for i = 1:p % iterate through decision makers
    for k = 1:n
        for j = 1:m % iterate through every feedback ***** O^3 *****
            X = [X;D_by_C_by_A_envelope{i,1}{j,k}];
        end
